...
- Phase 16 (Artifact Downloads & History Visibility) implemented, verified, and merged on 2026-05-29 (branch `phase-16-artifact-downloads`, commit `66a4285`, PR #14 merged as `ef19a9c`). Adds a streaming artifact download route (`GET /api/runs/[runId]/artifacts/[artifactId]/download` — real disk filename, `Content-Type` from `mimeType` with `application/octet-stream` fallback, RFC 5987 `Content-Disposition`, `X-Content-Type-Options: nosniff`, `(id, runId)` ownership, missing/unsafe file → 404 `artifact_file_missing`, `fs.createReadStream` → Web stream so 5MB+ exports never load fully into memory) and a lazy metadata-only history route (`GET /api/runs/[runId]/artifacts/history?kind=&path=` — `createdAt`/`bytes`/`sha256`/`id`/`latest`, newest-first, no content fetch; `kind`/`path` are DB equality filters only). `RunExportsPane` now splits Run outputs vs Team files, adds a per-row Download link and a lazy history toggle, and states that only the latest file is kept on disk (past versions are metadata only; preview/download always show the current file). New pure helpers `groupArtifactHistory` / `attachmentContentDisposition`. Artifact rows stay append-only (no cleanup / dedupe / unique index / upsert), no markdown renderer (preview stays plain text). No schema migration, no new dependency. typecheck clean; 237 / 237 tests; `next build` PASS (2 new routes); prisma migrate status 4 migrations (0 added, schema unchanged); provider-free smoke 26 / 26. Plan in `PHASE16_PLAN.md`.
- Phase 17 (Exports Robustness & Degradation Hardening) implemented, verified, and merged on 2026-06-03 (branch `phase-17-export-hardening`, commit `b072f5b`, PR #15 merged as `23cef79`). Hardens the Phase 16 read/download paths: preview no longer reads the whole file (`createReadStream` of `PREVIEW_BYTE_BUDGET + 1` bytes → UTF-8-safe partial decode via the new pure `decodePreview`; additive `totalBytes` while keeping `totalChars` for the non-truncated case); download is now single-fd (`open → fstat → same-fd createReadStream`, removing the stat↔stream `Content-Length` TOCTOU, with stream-error logging and fd ownership handed to `autoClose` on success / closed only on a pre-stream failure); both routes discriminate `ENOENT`/`EACCES`/`SafePathError` in logs (runId/artifactId/code only) while keeping responses opaque; `loadRunArtifacts`/`loadArtifactHistory` gain `orderBy desc + take` caps (RUN 1000 / HISTORY 200); `RunExportsPane` download is now fetch-based (inline row error, blob URL revoke) with the truncated-preview notice preferring `totalBytes`. Artifact rows stay append-only; no schema migration, no new dependency (package.json test-script only). typecheck clean; 247 / 247 tests; `next build` PASS (route count unchanged); prisma migrate status 4 migrations (0 added, schema unchanged); provider-free smoke 27 / 27. API correctness review APPROVE, scope/UI/compat review PASS, §9 FileHandle checklist all pass. Plan in `PHASE17_PLAN.md`.
- Phase 18 (Artifact Lifecycle Visibility) implemented, verified, and merged on 2026-06-03 (branch `phase-18-artifact-lifecycle-visibility`, commit `415ff6e`, PR #16 merged as `82552e3`). Surfaces append-only artifact redundancy NON-DESTRUCTIVELY (Artifact rows are read-only this phase — nothing created/updated/deleted/upserted). New pure helper `artifactStats.ts` (`countVersionsByGroup` → per-`(kind, path)` `versionCount`/`redundantCount` + optional sha256 breakdown `changedVersionCount`/`sameContentCount`; `summarizeRedundancy` → run-level `groups`/`totalVersions`/`redundant`/`redundancyPct`/`byKind`). `loadRunArtifacts` attaches these as additive optional `ArtifactMeta` fields computed from the rows it already fetched (no extra query, no new endpoint); `RunExportsPane` shows a run-level redundancy summary and a per-row `N versions · N redundant (metadata only)` label (download/copy-path/history/preview unchanged; consistent with the existing "past versions are metadata only / not recoverable" message). No schema migration, no new dependency (package.json test-script only). typecheck clean; 263 / 263 tests (16 new); `next build` PASS (route count unchanged); prisma migrate status 4 migrations (0 added, schema unchanged); provider-free smoke 17 / 17. API-correctness + non-destructive review APPROVE (zero artifact write verbs in the diff), scope/UI/compat review PASS. Plan in `PHASE18_PLAN.md`. Cleanup/dedupe/`@@unique`/upsert, dry-run cleanup planner, cross-run/admin dashboard, zip/bundle downloads, and markdown renderer remain deferred (Phase 19+).
- Phase 19 (Dry-Run Cleanup Planner) implemented, verified, and merged on 2026-06-03 (branch `phase-19-cleanup-planner`, commit `cc591eb`, PR #17 merged as `055ea6e`). A NON-DESTRUCTIVE, read-only preview of what a FUTURE keep-latest cleanup would remove — no deletion/dedupe/upsert/unique, no delete button, Artifact/RunEvent rows read-only this phase. New pure helper `artifactCleanupPlan.ts` (`planGroupCleanup` → per-(kind,path) keep-latest id / older metadata rows / represented historical bytes / same-content vs changed-version split / loaded event links affected; `summarizeCleanupPlan` → run-level totals). `loadRunArtifacts` attaches additive `representedHistoricalBytes` (group totalBytes − latest.bytes, from the rows it already fetched — no extra query, no new endpoint); the state route adds the existing `artifactId` to its event mapping (additive, no select/query change) and `RunStream` derives a memoized `loadedLinkedArtifactIds` set passed to `RunExportsPane`, which shows a run-level "Cleanup preview (no changes made)" summary and a per-row preview inside the history panel. Link impact is deliberately scoped to "loaded/visible event links" (polling path only; full DB audit deferred to Phase 20+). Wording uses "represented historical bytes" / "older metadata rows" / "not reclaimable disk" — never "would free / reclaim / save space" (past files are already overwritten; only DB metadata rows remain). No schema migration, no new dependency (package.json test-script only). typecheck clean; 279 / 279 tests (16 new); `next build` PASS (route count unchanged); prisma migrate status 4 migrations (0 added, schema unchanged); provider-free read-only smoke 15 / 15. Correctness + non-destructive review APPROVE (zero artifact/RunEvent write verbs, zero delete/execute CTA), scope/wording/UI-safety review PASS. Plan in `PHASE19_PLAN.md`. Actual cleanup/dedupe (+backup), server-side full RunEvent link audit, `@@unique`+upsert, cross-run dashboard, zip/bundle downloads, and markdown renderer remain deferred (Phase 20+).
- Phase 20 (Server-Side RunEvent Link Audit) implemented, verified, and merged on 2026-06-03 (branch `phase-20-artifact-link-audit`, commit `e2861e5`, PR #18 merged as `b3faf52`). Completes Phase 19's "loaded/visible" link signal with an authoritative, full-DB count — still NON-DESTRUCTIVE and read-only (no delete/dedupe/upsert/unique, no delete button; Artifact/RunEvent rows read-only). New pure helper `artifactLinkAudit.ts` (`selectCleanupCandidates` reuses `groupArtifactHistory` so the kept-latest row is never a candidate; `buildLinkAudit` folds an artifactId→count map into per-group/run-level totals). New read-only route `GET /api/runs/[runId]/artifacts/cleanup-audit` (the only new endpoint): `prisma.artifact.findMany` (runId-scoped) → non-latest candidates → `prisma.runEvent.groupBy({ by:['artifactId'], where:{ runId, type:'result.created', artifactId:{ in } }, _count })` (skipped when no candidates; null artifactId filtered) → JSON `{ summary, groups }` (counts only; candidateIds kept internal for Phase 21 reuse). Unknown run → zeros (200). `RunExportsPane` adds an opt-in "run full link audit" button showing `Loaded links: N · Full audit links: M · These links would be set to null by a future cleanup · RunEvent links only apply to result artifacts today` — Phase 19 loaded vs Phase 20 full distinguished; no delete/execute CTA; never "would free / reclaim / save space". No schema migration, no new dependency (package.json test-script only). typecheck clean; 294 / 294 tests (15 new); `next build` PASS (1 new read-only route registered); prisma migrate status 4 migrations (0 added, schema unchanged); provider-free read-only smoke 11 / 11. Correctness + read-only review PASS (zero artifact/RunEvent write verbs, zero delete/execute CTA), scope/route/wording review PASS. Plan in `PHASE20_PLAN.md`. Actual cleanup/dedupe (+backup), a destructive `artifacts.cleaned` audit-trail event, `@@unique`+upsert, cross-run dashboard, zip/bundle downloads, and markdown renderer remain deferred (Phase 21+).
...
## Phase 20 — Server-Side RunEvent Link Audit (2026-06-03)

Status: Implemented, verified, and merged. Branch `phase-20-artifact-link-audit`. Plan: `PHASE20_PLAN.md`. Commit `e2861e5`; PR #18 merged as `b3faf52`.

Completes Phase 19's dry-run planner by replacing the partial "loaded/visible" link signal with an authoritative, full-DB RunEvent link count. Core invariant — **Artifact and RunEvent rows are read-only this phase**; the one new route is read-only too. No deletion/dedupe/upsert/unique, no delete/execute button. schema migration 0 / dependency 0 / new endpoint = one read-only route.

### New files

- `src/lib/results/artifactLinkAudit.ts` (+ test) — pure, prisma-free, read-only:
  - `selectCleanupCandidates(rows)` → reuses `groupArtifactHistory` (newest-first; avoids re-implementing `isNewer`) to produce per-`(kind, path)` `{ latestId, candidateIds }` (non-latest ids) plus a flat `allCandidateIds`. The kept-latest row is structurally never a candidate.
  - `buildLinkAudit(groups, linkCountByArtifactId)` → per-group `{ candidateCount, linkedEventsAffected = Σ (map.get(id) ?? 0) }` + run-level `{ totalGroups, cleanupCandidateRows, affectedGroups, fullAuditLinkedEvents }`. A candidate absent from the map contributes 0 (non-result kinds / no links).
  - 15 unit tests (empty / single / multi-version / multiple groups / same-path-different-kind / linked old / linked latest-only / count>1 / non-result kind / aggregate totals / candidateIds carried / no-mutation).

### New route

- `app/api/runs/[runId]/artifacts/cleanup-audit/route.ts` — read-only `GET` (the only new endpoint). `prisma.artifact.findMany` (runId-scoped, take cap) → `selectCleanupCandidates` → `prisma.runEvent.groupBy({ by:['artifactId'], where:{ runId, type:'result.created', artifactId:{ in: allCandidateIds } }, _count:{ _all:true } })` (skipped when there are no candidates; null artifactId filtered) → `buildLinkAudit` → JSON `{ summary, groups }`. Response is count-centric — `candidateIds` stay internal (Phase 21 reuse / tests). Unknown/empty run → zeros (200). STRICTLY read-only: `findMany` + `groupBy` only, both runId-scoped; no create/update/delete/upsert.

### Modified files

- `src/components/run/RunExportsPane.tsx` — adds an opt-in "run full link audit" button (read-only GET, disabled after load) and a result line: `Loaded links: N · Full audit links: M · These links would be set to null by a future cleanup · RunEvent links only apply to result artifacts today`. Distinguishes the Phase 19 loaded/visible count from the Phase 20 authoritative full count. Display-only — NO delete/execute/apply button, no mutation; never "would free / reclaim / save space". Phase 18/19 displays + download / copy-path / history / preview unchanged.
- `apps/web/package.json` — registered `artifactLinkAudit.test.ts` (test-script list only; no dependency change).

### Verification

- typecheck PASS (0 errors); 294 / 294 tests (279 + 15 new); `next build` PASS (1 new read-only route registered, schema unchanged); `prisma migrate status` 4 migrations (0 added).
- Provider-free READ-ONLY smoke 11 / 11 (`apps/web/smoke-p20.mts`, created → run → deleted, not committed): (A) read-only cross-check — the route's `summary` (`totalGroups` / `cleanupCandidateRows` / `fullAuditLinkedEvents`) matches a raw `artifact.findMany` grouping + raw `runEvent.groupBy(type:'result.created')`, and the response omits `candidateIds`; (B) temp multi-version `result_md` + a temp `result.created` RunEvent linking a NON-latest artifact → that group's `candidateCount` 2 / `linkedEventsAffected` 1; (C) temp non-result kind → `candidateCount` 1 / `linkedEventsAffected` 0; temp RunEvent + artifacts deleted in `finally` with 0 leftovers (existing rows untouched).

### Code review (read-only)

- Correctness + read-only: PASS — zero artifact/RunEvent write verbs (create/update/delete/upsert) in the diff; helper pure (no I/O, no mutation); `latestId` never in `candidateIds`; no NaN/negatives; map default `?? 0`. Two MEDIUM notes (audit `onClick` missing `void`; missing count>1 test) were addressed before commit. No Critical/High.
- Scope / route / wording: PASS — exactly 5 changed files; exactly one new route (GET, read-only, runId-scoped, candidateIds omitted from response); schema / migrations / lockfile / 4 artifact creation sites / `report.md`+`exportReports` / `artifactList`+`artifactStats`+`artifactCleanupPlan`+`runArtifacts` / `PHASE_LOG.md` / `PHASE19_PLAN.md` untouched; no forbidden wording; no delete/execute CTA; preview/history/copy-path/download no regression.

### Constraints honored

- schema migration 0; new dependency 0; new endpoint = one read-only `cleanup-audit` route; Artifact + RunEvent rows read-only.
- No cleanup / delete / dedupe / update / upsert / unique; no delete or execute button.
- Artifact creation sites, `report.md`/`exportReports`, `artifactList`/`artifactStats`/`artifactCleanupPlan`/`runArtifacts`, schema / migrations / lockfile unmodified. No zip/bundle, markdown renderer, or cross-run dashboard.

### Deferred (Phase 21+)

- Actual cleanup / dedupe (with backup first); a destructive `artifacts.cleaned` audit-trail event recording what was removed.
- `@@unique([runId, kind, path])` + creation-site upsert; cross-run / admin artifact dashboard.
- Run zip/bundle downloads; markdown preview renderer.

## Phase 19 — Dry-Run Cleanup Planner (2026-06-03)

Status: Implemented, verified, and merged. Branch `phase-19-cleanup-planner`. Plan: `PHASE19_PLAN.md`. Commit `cc591eb`; PR #17 merged as `055ea6e`.

A NON-DESTRUCTIVE, read-only preview of what a FUTURE keep-latest cleanup would remove from the append-only Artifact rows. Core invariant — **Artifact and RunEvent rows are read-only this phase**: nothing is deleted/deduped/updated/upserted, no `@@unique`, no delete/execute button. schema migration 0 / dependency 0 / new endpoint 0.

### New files

- `src/lib/results/artifactCleanupPlan.ts` (+ test) — pure, prisma-free, read-only:
  - `planGroupCleanup(versions, linkedArtifactIds?)` → per-`(kind, path)` `{ keepLatestId, olderRows, representedHistoricalBytes, changedVersions?, sameContentRows?, loadedLinkedEventsAffected }`. `versions` is newest-first (as `groupArtifactHistory`/`loadArtifactHistory` return); older = non-latest rows; the sha256 split (changed vs same-content) is over the older rows' non-null sha.
  - `summarizeCleanupPlan(metas, linkedArtifactIds?)` → run-level `{ totalGroups, cleanupCandidateRows, affectedGroups, representedHistoricalBytes, loadedLinkedEventsAffected }`. loadedLinkedEventsAffected = loaded linked ids that are NOT a kept-latest id.
  - 16 unit tests (empty / single / same-sha / changed-sha / null-sha / mixed-null / represented sum / linked non-latest / linked latest-only / no-mutation; summarize empty / single / missing-versionCount / mixed / linked / no-mutation).

### Modified files

- `src/lib/results/runArtifacts.ts` — `ArtifactMeta` gains additive optional `representedHistoricalBytes` (= group total bytes − latest row bytes = Σ non-latest bytes), computed from the rows `loadRunArtifacts` already fetched (no extra query, no new endpoint). Read-only; `loadArtifactHistory` unchanged.
- `app/api/runs/[runId]/state/route.ts` — adds the existing `RunEvent.artifactId` to the events response mapping (additive; `findMany` has no `select`, so no query change). No new endpoint.
- `src/components/run/RunStream.tsx` — `InitialEvent` gains optional `artifactId`; a memoized `loadedLinkedArtifactIds` set (artifact ids of loaded events; polling path only) is passed to `RunExportsPane`. Reducer / SSE / activity timeline unchanged.
- `src/components/run/RunExportsPane.tsx` — when open, renders a run-level "Cleanup preview (no changes made)" summary and a per-row cleanup preview inside the history panel (above the version list). Display-only: NO delete/execute button, no `onClick`, counts render from props (no new fetch). Wording uses "represented historical bytes" / "older metadata rows" / "loaded event links would be nulled" / "a future cleanup would collapse history to latest-only" / "not reclaimable disk" — never "would free / reclaim / save space" (past files are already overwritten; only DB metadata rows remain). Consistent with Phase 16/18's "past versions are metadata only — content not recoverable". Download / copy-path / history / preview unchanged.
- `apps/web/package.json` — registered `artifactCleanupPlan.test.ts` (test-script list only; no dependency change).

### Verification

- typecheck PASS (0 errors); 279 / 279 tests (263 + 16 new); `next build` PASS (route count unchanged, no new endpoint); `prisma migrate status` 4 migrations (0 added, schema unchanged).
- Provider-free READ-ONLY smoke 15 / 15 (`apps/web/smoke-p19.mts`, created → run → deleted, not committed): (A) read-only cross-check — `summarizeCleanupPlan` totals + per-group `representedHistoricalBytes` match a raw `findMany` grouping for an existing run; (B) temp deterministic case — multi-version (sha A,A,B) + one temp `result.created` RunEvent linking a non-latest temp artifact → `planGroupCleanup` (keepLatest, olderRows 2, represented 30, changed 1, same-content 1, loadedLinkedEventsAffected 1), summary loadedLinkedEventsAffected 1, meta versionCount 3 / represented 30; temp RunEvent + artifacts deleted in `finally` with 0 leftovers (existing rows untouched).

### Code review (read-only)

- Correctness + non-destructive: APPROVE — zero artifact/RunEvent write verbs (create/update/delete/upsert) in the diff; helpers pure (no I/O, no mutation); `representedHistoricalBytes` math verified; no NaN/negatives. A MEDIUM perf note (the linked-ids Set rebuilt every render) was addressed with `useMemo` before commit. No Critical/High.
- Scope / wording / UI-safety: PASS — exactly 7 changed files; schema / migrations / lockfile / 4 artifact creation sites / `report.md`+`exportReports` / `artifactStats` / `artifactList` / `PHASE_LOG.md` / `PHASE18_PLAN.md` / artifacts routes untouched; no forbidden wording ("would free/reclaim/save space"); no delete/execute CTA; preview / history / copy-path / download no regression.

### Constraints honored

- schema migration 0; new dependency 0; new endpoint 0; Artifact + RunEvent rows read-only.
- No cleanup / delete / dedupe / update / upsert / unique; no delete or execute button.
- Artifact creation sites, `report.md`/`exportReports`, `artifactStats`/`artifactList`, schema / migrations / lockfile unmodified. No zip/bundle, markdown renderer, or cross-run dashboard.

### Deferred (Phase 20+)

- Actual cleanup / dedupe (with backup first); server-side full RunEvent link audit (vs the loaded/visible subset shown here).
- `@@unique([runId, kind, path])` + creation-site upsert; cross-run / admin artifact dashboard.
- Run zip/bundle downloads; markdown preview renderer.

## Phase 18 — Artifact Lifecycle Visibility (2026-06-03)

Status: Implemented, verified, and merged. Branch `phase-18-artifact-lifecycle-visibility`. Plan: `PHASE18_PLAN.md`. Commit `415ff6e`; PR #16 merged as `82552e3`.

Surfaces the append-only Artifact redundancy (current dev.db ~49%) NON-DESTRUCTIVELY: per-`(kind, path)` version counts and a run-level redundancy summary in the exports panel. Core invariant — **Artifact rows are read-only this phase**: nothing is created, updated, deleted, or upserted. schema migration 0 / dependency 0 / new endpoint 0.

### New files

- `src/lib/results/artifactStats.ts` (+ test) — pure, prisma-free, I/O-free:
  - `countVersionsByGroup(rows)` → `Map` keyed by `${kind} ${path}` (same key as `selectLatestArtifacts`/`groupArtifactHistory`) with `versionCount` (= group size), `redundantCount` (= max(0, versionCount − 1)), and an OPTIONAL sha256 breakdown — `changedVersionCount` (distinct non-null sha256) and `sameContentCount` (non-null rows − distinct), present only when the group has ≥1 non-null sha256 (older rows may predate sha tracking; null-sha rows are excluded from the breakdown).
  - `summarizeRedundancy(metas)` → run-level `{ groups, totalVersions, redundant, redundancyPct (1 decimal), byKind }`. Empty → zeros; missing `versionCount` treated as 1.
  - 16 unit tests (empty, single, multi same/changed/all-distinct sha, null-only, mixed null, multiple groups, byKind, rounding, no-mutation).

### Modified files

- `src/lib/results/runArtifacts.ts` — `ArtifactMeta` gains additive optional `versionCount` / `redundantCount` / `sameContentCount` / `changedVersionCount`. `loadRunArtifacts` computes these via `countVersionsByGroup` over the SAME rows it already fetched (no extra query, no new endpoint) and attaches them to each latest-per-group metadata. Strictly read-only — no artifact write. `loadArtifactHistory` unchanged. The new fields flow to the client through the existing state route / RunStream / page (which pass artifacts through unchanged).
- `src/components/run/RunExportsPane.tsx` — local `ArtifactMeta` gains the same optional fields; imports the pure `summarizeRedundancy`. When open, renders a run-level summary line (`N files · N versions · N redundant (N%)`); per row with `versionCount > 1`, renders `N versions · N redundant (metadata only)` (+ optional `· M same content`) on its own line after the path. Counts render from props (no new fetch/state). Download (button+fetch), copy-path, history toggle, and preview are unchanged; the per-row label complements the history panel's existing "past versions are metadata only / not recoverable" disclaimer.
- `apps/web/package.json` — registered `artifactStats.test.ts` (test-script list only; no dependency change).

### Verification

- typecheck PASS (0 errors); 263 / 263 tests (247 + 16 new); `next build` PASS (route count unchanged, no new endpoint); `prisma migrate status` 4 migrations (0 added, schema unchanged).
- Provider-free smoke 17 / 17 (`apps/web/smoke-p18.mts`, created → run → deleted, not committed): (A) read-only cross-check — `loadRunArtifacts` `versionCount`/`redundantCount` and `summarizeRedundancy` totals match a raw `findMany` grouping for an existing run; (B) temp-row deterministic case — multi-version (sha A,A,B) → versionCount 3 / redundant 2 / changedVersionCount 2 / sameContentCount 1, single-version → 1/0/0, latest = newest row; temp rows deleted in `finally` with 0 leftovers (existing rows untouched).

### Code review (read-only)

- API correctness + non-destructive: APPROVE — zero artifact write verbs (create/update/delete/upsert) in the diff; helpers pure (no I/O, no mutation); group key matches latest-selection; sha breakdown / rounding / null-sha branches correct; no extra query. No Critical/High.
- Scope / UI / compatibility: PASS — exactly 5 changed files; schema / migrations / lockfile / 4 artifact creation sites / `report.md`+`exportReports` / `PHASE_LOG.md` / `PHASE17_PLAN.md` / artifacts routes / state route / RunStream / page untouched; response additive (no client crash); preview/history/copy-path/download no regression; metadata-only messaging consistent.

### Constraints honored

- schema migration 0; new dependency 0; new endpoint 0; Artifact rows read-only.
- No cleanup / dedupe / delete / update / upsert / unique. Artifact creation sites, `report.md`/`exportReports`, schema / migrations / lockfile unmodified. No zip/bundle or markdown renderer.

### Deferred (Phase 19+)

- Cleanup / dedupe / `@@unique([runId, kind, path])` / upsert (needs backup + RunEvent.artifactId audit + dedup-before-migration; conflicts with Phase 16 history).
- Dry-run cleanup planner; cross-run / admin artifact dashboard.
- Run zip/bundle downloads; markdown preview renderer.

## Phase 17 — Exports Robustness & Degradation Hardening (2026-06-03)

Status: Implemented, verified, and merged. Branch `phase-17-export-hardening`. Plan: `PHASE17_PLAN.md`. Commit `b072f5b`; PR #15 merged as `23cef79`.

Hardens the Phase 16 read/download paths (preview unbounded `readFile`, download `stat`↔`stream` TOCTOU) and adds query caps plus degraded-path logging. Robustness only — no new capability surface. schema migration 0 / dependency 0.

### New files

- `src/lib/results/artifactPreview.ts` (+ test) — pure `decodePreview(buf, hadMore, maxChars)`: when the buffer was byte-truncated (`hadMore`) it drops an incomplete trailing UTF-8 sequence before decoding, then applies the char cap; byte-budget and char-cap truncation fold into one `truncated` flag. No U+FFFD boundary damage. prisma-free; 10 tests (ASCII under/over cap, empty, `hadMore` alone, exact cap, char+byte simultaneous, complete multibyte, multibyte/emoji boundary cut, negative cap).

### Modified files

- `app/api/runs/[runId]/artifacts/[artifactId]/route.ts` (preview) — removes the whole-file `readFile`; reads `PREVIEW_BYTE_BUDGET + 1` bytes via `createReadStream({ start: 0, end: budget })` (inclusive `end`; receiving `> budget` ⇒ `hadMore`), decodes only the first `budget` bytes through `decodePreview`. Response is additive: `truncated=false` → `totalChars` (exact); `truncated=true` → `totalBytes` (authoritative, from the DB row) with `totalChars` omitted. Missing/unsafe still degrades to `{ missing: true }`.
- `app/api/runs/[runId]/artifacts/[artifactId]/download/route.ts` — single file descriptor: `open → fstat → fh.createReadStream({ autoClose: true })`, so `Content-Length` (from the same-fd `fstat`) matches the streamed bytes (stat↔stream TOCTOU removed). fd ownership is handed to `autoClose` on success (`fh` nulled right after stream creation so the catch never double-closes); `fh` is closed only on a pre-stream failure (`safeJoin`/`open`/`fstat`/non-file). A stream `error` handler logs a stable identifier (no path/content) without mutating the already-started response. Headers (`nosniff`, `Content-Disposition`, `Cache-Control`) unchanged.
- both routes — discriminate `ENOENT` (warn) vs `EACCES`/other (error) vs `SafePathError` (logged as `ESAFEPATH`); logs carry only `runId`/`artifactId`/`code` (no path, no content). Client responses stay opaque (preview `{ missing: true }` / 200; download 404 `artifact_file_missing`).
- `src/lib/results/runArtifacts.ts` — `loadRunArtifacts` and `loadArtifactHistory` `findMany` gain `orderBy: { createdAt: 'desc' }` + `take` caps (`RUN_ARTIFACTS_MAX = 1000` / `HISTORY_MAX = 200`); desc+take preserves the "latest N" semantics the in-memory `selectLatestArtifacts` / `groupArtifactHistory` rely on.
- `src/components/run/RunExportsPane.tsx` — download is now a fetch-based handler (a `<button>` replaces the `<a>`): a non-OK response renders an inline per-row error instead of opening a 404 JSON in a tab; success saves the blob via a transient object URL that is revoked immediately (filename from `rowName`). The truncated-preview notice prefers `totalBytes` (falls back to `totalChars`); `ContentState` gains `totalBytes?`. Preview / history / copy-path untouched.
- `apps/web/package.json` — registered `artifactPreview.test.ts` (test-script list only; no dependency change).

### Verification

- typecheck PASS (0 errors); 247 / 247 tests (237 + 10 new); `next build` PASS (route count unchanged); `prisma migrate status` 4 migrations (0 added, schema unchanged).
- Provider-free smoke 27 / 27 (`apps/web/smoke-p17.mts`, created → run → deleted, not committed): preview small/large/multibyte, download length/body/`nosniff`/missing(404)/cross-run(404), history take(200)/empty/400; dev.db temp rows + files cleaned to 0 leftovers.

### Code review (read-only)

- API correctness: APPROVE — §9 FileHandle-lifetime checklist all pass (success path never closes `fh`; pre-stream failure closes it; stream-error log-only; same-fd `Content-Length` / TOCTOU removed; opaque responses; ENOENT/EACCES discrimination without leak). No Critical/High.
- Scope / UI / compatibility: PASS — exactly 7 changed files; schema / migrations / lockfile / artifact-creation sites / `PHASE_LOG.md` untouched; response change additive (no client crash); preview / history / copy-path no regression.
- One Low note (`Readable.toWeb` throwing after `fh = null`) is structurally unreachable with the standard library and is recorded in `PHASE17_PLAN.md` §7(f).

### Constraints honored

- schema migration 0; new dependency 0; artifact creation sites unmodified; `schema.prisma` / migrations / lockfile unmodified.
- No lifecycle visibility, cleanup, dedupe, unique index, upsert, zip/bundle, or markdown renderer.

### Deferred (Phase 18+)

- Artifact lifecycle visibility / redundancy dashboard; cleanup / dedupe / `@@unique([runId, kind, path])` / upsert.
- Run zip/bundle downloads; markdown preview renderer.
- Deeper stream observability (metrics / structured logging) if needed.

## Phase 16 — Artifact Downloads & History Visibility (2026-05-29)

Status: Implemented, verified, and merged. Branch `phase-16-artifact-downloads`. Plan: `PHASE16_PLAN.md`. Commit `66a4285`; PR #14 merged as `ef19a9c`.

### Approved scope (decisions)

1. **Download = separate streaming route, real disk filename.** `GET /api/runs/[runId]/artifacts/[artifactId]/download` streams the original bytes via `fs.createReadStream` → Web stream (5MB+ safe), keeps the actual on-disk filename (`result.md` / `report.md` / `{agentId}.md` / `team.json`), sets `Content-Type` from `artifact.mimeType` (`application/octet-stream` fallback), an RFC 6266/5987-safe `Content-Disposition` (`filename` ASCII fallback + `filename*` for non-ASCII, header-injection safe), and `X-Content-Type-Options: nosniff`. Ownership via `findFirst({ id, runId })`; missing/unsafe file → 404 `artifact_file_missing` (no 500, no path leak).
2. **History visibility = metadata-only, lazy.** `GET /api/runs/[runId]/artifacts/history?kind=&path=` returns every append-only version for one `(kind, path)` (newest-first) with `createdAt` / `bytes` / `sha256` / `id` / `latest` — **no content fetch**. `kind` / `path` are DB equality filters only (no fs access, no cross-run leak). Missing params → 400.
3. **`Artifact` policy = append-only preserved.** No cleanup / dedupe / `(runId, kind, path)` unique index / upsert; duplicate rows are surfaced as creation history at the display layer. schema migration 0.
4. **Markdown renderer not introduced.** Preview stays plain-text `<pre>` (markdown source); JSON artifacts (`team_json`) preview the same way. dependency 0.
5. **Team files split.** `RunExportsPane` separates **Run outputs** (`result_md` / `report_md` / `agent_report_md`) from **Team files** (`team_md` / `team_json`), adds a per-row Download link and a lazy history toggle, and states that only the latest file is kept on disk (past versions are metadata only; preview/download always show the current file).

### New files

- `src/lib/results/contentDisposition.ts` (+ test) — pure `attachmentContentDisposition(filename)`: RFC 6266/5987 builder; strips control chars (incl. CR/LF), neutralizes quotes/backslash, percent-encodes `filename*`; empty/whitespace/control-only input falls back to `download` in both forms. prisma-free.
- `app/api/runs/[runId]/artifacts/[artifactId]/download/route.ts` — streaming download (ownership + `safeJoin` + `fs.stat`/`isFile` + `createReadStream` → Web stream + headers; missing/unsafe → 404 `artifact_file_missing`).
- `app/api/runs/[runId]/artifacts/history/route.ts` — lazy metadata-only history list for one `(kind, path)`; missing params → 400.

### Modified files

- `src/lib/results/artifactList.ts` (+ test) — adds pure `groupArtifactHistory(rows)`: groups append-only rows by `(kind, path)` into `{ latest, versions[] }` (newest-first, deterministic id tie-break, same display order as `selectLatestArtifacts`). Existing `selectLatestArtifacts` unchanged. Does not mutate the caller's input array.
- `src/lib/results/runArtifacts.ts` — adds `loadArtifactHistory(runId, kind, path)` (metadata-only, via `groupArtifactHistory`). `loadRunArtifacts` unchanged.
- `src/components/run/RunExportsPane.tsx` — Run outputs / Team files sections, per-row Download link (`<a download target="_blank" rel="noopener">` so a missing-file 404 never replaces the run page), lazy history toggle with a "content not recoverable" notice above the version list + `metadata only` tag on non-latest rows. Existing preview / copy-path preserved.
- `apps/web/package.json` — registered `contentDisposition.test.ts` (test-script list only; no dependency change).

### Verification

- `corepack pnpm --filter web typecheck` — PASS (0 errors).
- `corepack pnpm --filter web test` — PASS, 237 / 237 (Phase 15 baseline 218 + 19 new: `groupArtifactHistory` 7, `contentDisposition` 12 incl. empty/whitespace/control/semicolon/percent/backslash + injection cases).
- `next build` — PASS, 2 new routes (`/artifacts/[artifactId]/download`, `/artifacts/history`).
- `prisma migrate status` — PASS, 4 migrations, schema unchanged (no migration added).
- Code review — four parallel read-only review agents (API/security, UI/UX, test/coverage, scope/regression); 0 blocking. Surgical hardening applied: `nosniff` header; download link `target="_blank" rel="noopener"`; history notice moved above the version list; `contentDisposition` empty/whitespace/control fallback aligned across both filename forms.
- Provider-free smoke (route handlers against real dev.db, temp rows/files cleaned up) — 26 / 26: md/json download (Content-Type, Content-Length, `nosniff`, body sha256 == disk), unknown-id 404, cross-run ownership 404, missing-on-disk 404, history happy path (count/desc/single-latest/latest-first), empty-match history → 200 `{ versions: [] }`, missing params → 400, non-ASCII filename download (`filename*` percent-encoded + ASCII fallback + sha256). dev.db artifact count unchanged (75), 0 leftover rows/dirs.

### Deferred (Phase 17+)

- Artifact cleanup / `(runId, kind, path)` upsert / unique index.
- Markdown renderer (preview stays plain text).
- Eager `history (k)` version count in the toggle label (would need a new server-side `ArtifactMeta` field).
- Download robustness hardening: `stat` ↔ stream `Content-Length` TOCTOU, mid-stream error/abort logging.
...
- Live guard smoke without provider cost: `/resume` 404/409 guards, task-retry 404 guard, and Phase 6/7 screen no-regression checks.
- Provider-backed partial-resume smoke: created a synthetic failed run (`cmpk0uedb000187m4e9nz8yg5`) from an existing succeeded Gemini run, with one done seed task and one failed dependent worker task. Browser Resume moved the run to `running`, reused the done task output as dependency context, re-ran only the failed task, then reached `succeeded` with `result.md`, `report.md`, and `agent-reports/{agentId}.md` artifacts.
